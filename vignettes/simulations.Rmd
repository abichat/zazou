...
estimation_scla <- estimate_shifts(rep(0, N_branch), zscores = zscores, tree = tree, 
                                   alpha = estimation_shooting$alpha, 
                                   lambda = estimation_shooting$lambda, 
                                   method = "scaledlasso", constraint_type = "beta")

desparsified <- estimate_confint(estimation_scla, method = "desparsified")
toc()
estimation_shooting
desparsified
...
ebc_tidy(detected_sh, otus_da, m = length(otu_to_keep), measures = )
...
map_dfr(list(detected_sh, detected_bh, c(), otu_to_keep), ebc_tidy, 
...
several_confint <-
  ...
  mutate(model = map(level, update_confint, x = desparsified),

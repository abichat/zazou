...
estimation <- estimate_shifts3(rep(0, N_branch), zscores = zscores, 
                               lambda = c(0.1, 0.5, 1, 2, 5, 7, 10), tree = tree, alpha = c(0.01, 0.1, 1, 5))
estimation
plot(estimation, 
     sup_scores = list(list(scores = zscores - zscores_original,
                            title = "Difference in z-scores after fold-change",
...
evabic::ebc_tidy(detected, otus_da, m = length(otu_to_kepp), measures = c("TPR", "FDR", "ACC", "F1", "BACC"))
evabic::ebc_tidy(detected_bh, otus_da, m = length(otu_to_kepp), measures = c("TPR", "FDR", "ACC", "F1", "BACC"))
```

...
estS <- estimate_shifts3(Delta0 = rep(0, N_branch), zscores = zsco_obs,
                         lambda = grid, tree = tree,
                         alpha = grid, method = "shooting3")
...
estL <- estimate_shifts3(Delta0 = rep(0, N_branch), zscores = zsco_obs,
                         lambda = grid, tree = tree,
                         alpha = grid, method = "L-BFGS-B")

estL_best <- estimate_shifts3(Delta0 = rep(0, N_branch), zscores = zsco_obs,
                              lambda = estL$lambda, tree = tree,
                              alpha = estL$alpha, method = "L-BFGS-B")

estL_notbest <- estimate_shifts3(Delta0 = rep(0, N_branch), zscores = zsco_obs,
